...
def test_custom_engine_proposal_is_applied_when_better():
    ...
    out = reflow_line_breaks(["I saw it, but", "she left."], [2.0, 2.0], resplitter=engine)
...
class _StubSplitter(ReflowSplitter):
    ...
    def _run_structured_output(self, **kwargs):
...
def test_llm_resplitter_falls_back_to_deterministic_on_error(monkeypatch):
    ...
    monkeypatch.setattr(
        reflow_llm.ReflowSplitter, "__init__", lambda self, **kw: None
    )
