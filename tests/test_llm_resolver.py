...
def test_resolver_rejects_unsupported_openrouter_only_model_for_now(monkeypatch):
    _clear_llm_env(monkeypatch)
    monkeypatch.setenv("OPENROUTER_API_KEY", "test-openrouter")

    with pytest.raises(LLMResolutionError, match="not supported"):
        resolve_llm_selection(model="qwen/qwen3.6-plus:free", provider="openrouter")


def test_resolver_does_not_treat_arbitrary_o_prefix_vendor_models_as_openai(
    monkeypatch,
):
    _clear_llm_env(monkeypatch)
    monkeypatch.setenv("OPENROUTER_API_KEY", "test-openrouter")
    ...
    with pytest.raises(LLMResolutionError, match="not supported"):
        resolve_llm_selection(model="openai/claude-sonnet-4-6", provider="openrouter")
