...
def _write_postprocessed(path, cues):
    from autosub.core.schemas import SubtitleDocument

    path.write_text(
        SubtitleDocument(stage="postprocessed", cues=cues).model_dump_json(indent=2),
        encoding="utf-8",
    )


def test_refinish_strips_corner_markers_and_closes_short_gaps(tmp_path):
    """Bring an episode made before those rules existed up to current output."""
    from autosub.core.schemas import SubtitleCue, SubtitleDocument

    doc_path = tmp_path / "Ep_postprocessed.json"
    map_path = tmp_path / "speaker_map.toml"
    _write_slot_speaker_map(map_path)
    _write_postprocessed(
        doc_path,
        [
            SubtitleCue(
                id="cue-00000001", start_time=0.0, end_time=2.0,
                speaker="Date Sayuri", source_text="ソース",
                final_text="[CORNER: Opening] Good evening.",
            ),
            # 120ms hand-off to the other slot: a flash, must close
            SubtitleCue(
                id="cue-00000002", start_time=2.12, end_time=4.0,
                speaker="Liyuu", source_text="ソース", final_text="Hello there.",
            ),
            # a real pause, must survive
            SubtitleCue(
                id="cue-00000003", start_time=6.0, end_time=8.0,
                speaker="Liyuu", source_text="ソース", final_text="Later on.",
            ),
        ],
    )

    result = runner.invoke(
        app, ["refinish", str(doc_path), "--speaker-map", str(map_path)]
    )
    assert result.exit_code == 0, result.output

    doc = SubtitleDocument.model_validate_json(doc_path.read_text(encoding="utf-8"))
    assert doc.cues[0].final_text == "Good evening."          # marker gone
    assert doc.cues[0].end_time == doc.cues[1].start_time      # gap closed
    assert doc.cues[1].end_time == 4.0                         # real pause kept
    assert doc.cues[2].start_time == 6.0
    assert (doc_path.parent / "Ep_final.ass").exists()


def test_refinish_is_idempotent(tmp_path):
    """Running it on an already-clean document must change nothing."""
    ...
    _write_postprocessed(
        ...
        [
            SubtitleCue(
                id="cue-00000001", start_time=0.0, end_time=2.0,
                speaker="Date Sayuri", source_text="ソース", final_text="Clean line.",
            ),
            SubtitleCue(
                id="cue-00000002", start_time=2.0, end_time=4.0,
                speaker="Liyuu", source_text="ソース", final_text="Chained already.",
            ),
        ],
    )
    args = ["refinish", str(doc_path), "--speaker-map", str(map_path)]
    assert runner.invoke(app, args).exit_code == 0
    first = doc_path.read_text(encoding="utf-8")
    assert runner.invoke(app, args).exit_code == 0
    assert doc_path.read_text(encoding="utf-8") == first
