...
def review_speakers(
    ...
    logger.info(f"Found {len(speaker_words)} unique speaker labels: {sorted(speaker_words.keys())}")
    ...
    logger.info(f"Loading speaker map from {speaker_map_path}...")
    ...
    logger.info(f"Speaker map contains {len(real_speakers)} unique speakers: {[s['name'] for s in real_speakers]}")
    ...
    if not assignments:
        logger.info("No assignments made. Speaker map unchanged.")
        raise typer.Exit()
    ...
    logger.info(f"Wrote speaker assignments to {output}")
