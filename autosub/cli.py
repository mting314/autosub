...
def _interactive_speaker_review(
    ...
    # Log summary
    skipped = [lbl for lbl in sorted_labels if lbl not in assignments]
    logger.info(
    ...
    lines = [
        "# Speaker assignments generated by review-speakers",
    ]
    if transcript_path:
...
@app.command(name="refinish")
def refinish(
    document: Path = typer.Argument(
        ...,
        help="Path to a translated or postprocessed JSON document, updated in place.",
        exists=True,
        dir_okay=False,
    ),
    speaker_map_path: Path = typer.Option(
        ...
        "--speaker-map",
        "-s",
        help="Path to speaker_map.toml, for the slot styles in the re-rendered .ass.",
        exists=True,
        dir_okay=False,
    ),
    ass_out: Path = typer.Option(
        None,
        "--ass-out",
        help="Where to write the re-rendered .ass (defaults beside the document).",
    ),
    min_duration_ms: int = typer.Option(
        500, "--min-duration-ms", help="Shortest a line may stay on screen."
    ),
    min_gap_ms: int = typer.Option(
        None,
        "--min-gap-ms",
        help="Shortest legible pause; below this a gap is closed. "
        "Defaults to --min-duration-ms.",
    ),
    bilingual: bool = typer.Option(
        False, "--bilingual/--replace", help="Render the source text above the line."
    ),
):
    """
    Re-apply the current output rules to a document produced by an older run.

    Episodes subtitled before a rule existed keep whatever the pipeline did at the
    time. This brings such a document up to date without re-transcribing or
    re-translating it:

    \b
      - removes the [CORNER: x] prefix the translator was asked to prepend, which
        was prose in the line rather than metadata and rendered on screen;
      - closes gaps too short to read as a pause, measured across every slot, so
        the screen never blinks empty for a few frames at a speaker hand-off.

    Then re-renders the .ass and reports anything still violating the rules.
    """
    ...
    from autosub.pipeline.translate.main import _CORNER_MARKER_RE

    doc = SubtitleDocument.model_validate_json(document.read_text(encoding="utf-8"))
    speaker_map = load_speaker_map(speaker_map_path) if speaker_map_path else None
    gap_floor = min_duration_ms if min_gap_ms is None else min_gap_ms

    # Whichever text field this stage renders from is the one that matters.
    text_field = "final_text" if doc.stage == "postprocessed" else "translated_text"
    ...
    for cue in doc.cues:
        for field in ("translated_text", "final_text"):
            ...
            if not value:
                continue
            cleaned = _CORNER_MARKER_RE.sub(r"\1", value)
            if cleaned != value:
    ...
    spans = [
        _Span(cue)
        for cue in doc.cues
        if (getattr(cue, text_field, None) or "").strip()
    ]
    before = [span.end_ms for span in spans]
    _close_screen_gaps(spans, gap_floor)
    extended = sum(1 for span, was in zip(spans, before) if span.end_ms != was)
    for span in spans:
        span.cue.end_time = span.end_ms / 1000.0

    document.write_text(doc.model_dump_json(indent=2), encoding="utf-8")

    if ass_out is None:
        suffix = "final" if doc.stage == "postprocessed" else doc.stage
        ass_out = document.with_name(
            f"{document.stem.rsplit('_', 1)[0]}_{suffix}.ass"
        )
    mode = "bilingual" if bilingual else (
        "final" if doc.stage == "postprocessed" else "translated"
    )
    render_ass_document(doc, ass_out, mode=mode, speaker_map=speaker_map)

    remaining = check_display_invariants(
        doc.cues,
    ...
    logger.info(
        "Refinished %s: removed %d corner marker(s), extended %d line(s) to close "
        "short gaps, re-rendered %s.",
        document.name,
        stripped,
        extended,
        ass_out.name,
    ...
    if remaining:
        logger.warning(
            "%d display invariant violation(s) remain (these need fixing upstream):",
            len(remaining),
        )
        ...
        try:
            ...
            if profile_data.get("corners"):
                ...
                for corner in profile_data["corners"]:
                    ...
                    if corner.get("cues"):
                        corners_text += (
                            f"  Common cue phrases: {', '.join(corner['cues'])}\n"
                        )
                corners_text += (
