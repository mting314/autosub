...
def reflow_line_breaks(
    ...
    for group, (pieces, _durs), new_pieces in zip(groups, batch, proposals):
        if new_pieces is None or not _accept_resplit(new_pieces, pieces):
            continue
    ...
    if reflowed_groups:
        logger.info("Line-break reflow adjusted %d sentence group(s).", reflowed_groups)
    return result
